...
if(isset($_POST['Reg_num']) && isset($_POST['email']) && isset($_POST['ID']) && isset($_POST['password']) && isset($_POST['submit'])) {
		...
		$timestamp = date('Y-m-d  H:i:s');
		 ...
		 if (!empty($Reg_num) &&!empty($email) &&!empty($ID) &&!empty($password)) {
			 ...
			 $sql_select1 = "SELECT `Reg` FROM pgd WHERE Reg LIKE '".mysqli_real_escape_string($conn, $Reg_num)."'";
			 $sql_select2 = "SELECT `email`  FROM pgd WHERE email LIKE '".mysqli_real_escape_string($conn, $email)."'";
			...
			if (  mysqli_num_rows($result1) === 0) {//reg check
				...
				echo '<script>document.getElementById("oz-header").innerHTML = "used ID!";</script>';
						...
						//INSERTING
				$insert = "UPDATE `pgd` SET `email`='".mysqli_real_escape_string($conn, $email)."',`serial`='".mysqli_real_escape_string($conn, $ID)."',`password`='".mysqli_real_escape_string($conn, $password_hash)."',`reg_date`='".$timestamp."' WHERE Reg='".mysqli_real_escape_string($conn, $Reg_num)."'";
				//INsert into ID
	 ...
	 //login
		...
		if(!empty($Reg_num) && !empty($password)){
			$sql_select = "SELECT `id` FROM pgd WHERE Reg LIKE '".mysqli_real_escape_string($conn, $Reg_num)."' AND password LIKE '".mysqli_real_escape_string($conn, $password_hash)."'";
		...
		$rows = mysqli_num_rows($login_query);
			...
			if ($rows==0) {
				...
				setcookie('Reg_num', $Reg_num, time() + 2000, '/');
				setcookie('password', $password_hash, time() + 2000, '/');
...
function UserId($get) {
	require '../functions/conn.php';
	$query = "SELECT ".$get." FROM pgd WHERE `id`='".$_SESSION['user_id']."'";
					...
					if ($filesize < 1000000) {
						if ($fileError === 0) {
							if (in_array($fileActualExt, $allowedExt) == true) {
								...
								//query
								$upload_insert = "INSERT INTO filepgd VALUES ('', '".$Reg_num."', '', '', '', '".$filename."')";
