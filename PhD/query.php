...
if(isset($_POST['Reg_num']) && isset($_POST['email']) && isset($_POST['ID']) && isset($_POST['password']) && isset($_POST['submit'])) {
		...
		$timestamp = date('Y-m-d  H:i:s');
		 ...
		 if (!empty($Reg_num) &&!empty($email) &&!empty($ID) &&!empty($password)) {
			 ...
			 $sql_select1 = "SELECT `Reg` FROM phd WHERE Reg LIKE '".mysqli_real_escape_string($conn, $Reg_num)."'";
			 $sql_select2 = "SELECT `email`  FROM phd WHERE email LIKE '".mysqli_real_escape_string($conn, $email)."'";
			...
			if (  mysqli_num_rows($result1) === 0) {//reg check
				...
				echo '<script>document.getElementById("oz-header").innerHTML = "used ID!";</script>';
						...
						//INSERTING
				$insert = "UPDATE `phd` SET `email`='".mysqli_real_escape_string($conn, $email)."',`serial`='".mysqli_real_escape_string($conn, $ID)."',`password`='".mysqli_real_escape_string($conn, $password_hash)."',`reg_date`='".$timestamp."' WHERE Reg='".mysqli_real_escape_string($conn, $Reg_num)."'";
				//INsert into ID
	 ...
	 //login
		...
		if(!empty($Reg_num) && !empty($password)){
			$sql_select = "SELECT `id` FROM phd WHERE Reg LIKE '".mysqli_real_escape_string($conn, $Reg_num)."' AND password LIKE 
...
function UserId($get) {
		require '../functions/conn.php';
	$query = "SELECT ".$get." FROM phd WHERE `id`='".$_SESSION['user_id']."'";
					...
					if ($filesize < 1000000) {
						if ($fileError === 0) {
							if (in_array($fileActualExt, $allowedExt) == true) {
								...
								//query
								$upload_insert = "INSERT INTO filephd VALUES ('', '".$Reg_num."', '', '', '', '".$filename."')";
